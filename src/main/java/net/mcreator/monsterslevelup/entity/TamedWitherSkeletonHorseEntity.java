...
public class TamedWitherSkeletonHorseEntity extends TamableAnimal implements GeoEntity {
	...
	public TamedWitherSkeletonHorseEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(MonstersLevelUpModEntities.TAMED_WITHER_SKELETON_HORSE.get(), world);
	}

	public TamedWitherSkeletonHorseEntity(EntityType<TamedWitherSkeletonHorseEntity> type, Level world) {
		super(type, world);
		xpReward = 8;
		setNoAi(false);
		setMaxUpStep(1f);
	...
	public SoundEvent getHurtSound(DamageSource ds) {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("monsters_level_up:entity.wither_skeleton_horse.hurt"));
	}

	@Override
	public SoundEvent getDeathSound() {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("monsters_level_up:entity.wither_skeleton_horse.death"));
	}
	...
	public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
		...
		TamedWitherSkeletonHorseRightClickedOnEntityProcedure.execute(world, entity, sourceentity);
		return retval;
	}
	...
	public void baseTick() {
		super.baseTick();
		TamedWitherSkeletonHorseTickProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		TamedWitherSkeletonHorseEntity retval = MonstersLevelUpModEntities.TAMED_WITHER_SKELETON_HORSE.get().create(serverWorld);
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F)) && !this.isVehicle() && !this.isAggressive()) {
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.WitherHorse_walk"));
			}
			if (this.isVehicle() && event.isMoving()) {
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.WitherHorse_aggressive"));
			}
			if (this.isAggressive() && event.isMoving() && !this.isVehicle()) {
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.WitherHorse_aggressive"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.WitherHorse_idle"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(TamedWitherSkeletonHorseEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
